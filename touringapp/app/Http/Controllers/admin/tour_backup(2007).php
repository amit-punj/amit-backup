...
class ToursController extends Controller
{
                                    ...
                                    foreach ($postData['variation_type'] as $key => $value) 
                                    {
                                        ...
                                        if($value == $v_key)
                                        {
                                            if ($postData['content_type'][$key] == 'image' && $request->hasFile('pic')) 
                                                ...
                                                {
                                                    $content = $postData['editor'][$editor_pic_index];
                                                }
    ...
    public function create(Request $request)
    {
        ...
        if($request->isMethod('post'))
        {
            ...
            print_r($postData);
            die('FFF');
            if(!empty($postData['v']) && isset($postData['v']))
            {
                foreach ($postData['v'] as $key => $value) 
                {
                    $index1 = explode("_",$key);
                    $index = $index1[0];
                    print_r($index[0]);
                    echo "/-------".$key."<br>";
                    $files = $request->file('pic');
                    if ($postData['content_type'][$index] == 'image' && $request->hasFile('pic')) 
                    {
                        $file = $files[$key];
                        $allowedfileExtension=['png'];
                        $filename = $file->getClientOriginalName();
                    ...
                    else if ($postData['content_type'][$index] == 'video' && $request->hasFile('pic')) 
                    {
                        $file = $files[$key];
                        $allowedfileExtension=['mp4'];
                        $filename = $file->getClientOriginalName();
                        ...
                        foreach ($postData['variations'] as $v_key => $v_value) 
                        {
                            ...
                            if($variations = Variations::create($variation_data))
                            {
                                ...
                                if(!empty($postData['v']) && isset($postData['v']))
                                {
                                    foreach ($postData['v'] as $key => $value) 
                                    {
                                        // $editor_pic_index = $key."_".$v_key;
                                        // echo $editor_pic_index;
                                        $index1 = explode("_",$key);
                                        $index = $index1[0];
                                        // die('amirt');
                                        $pic     = "";
                                        $content = "";
                                        // echo $value."==".$v_value."<br>";
                                        if($value == $v_value)
                                        {
                                            // echo $key."if".$value."<br>";
                                            if ($postData['content_type'][$index] == 'image' && $request->hasFile('pic')) 
                                                {       
                                                    $files = $request->file('pic');
                                                    $file = $files[$key];
                                            ...
                                            else if($postData['content_type'][$index] == 'text')
                                                {
                                            ...
                                            $poi_data = array(
                                                'tour_id'        => $tour_id,
                                                'variation_id'   => $variation_id,
                                                'poi_name'       => $postData['poi_name'][$index],
                                                'lat'            => $postData['lat'][$index],
                                                'long'           => $postData['long'][$index],
                                                'icon_type'      => $postData['icon_type'][$index],
                                                'content_type'   => $postData['content_type'][$index],
                                                'poi_id'         => $postData['poi_id'][$index],
                                                'content'        => $content,
                                                'image'          => $pic,
                                            );
                                            $poi = Poi::create($poi_data);
    ...
    public function update(Request $request, Tours $tour)
    {
        ...
        $poi_details = \DB::table('poi')
            ->Join('variations', 'variations.id', '=', 'poi.variation_id')
            ->select('variations.variation_name as variation_name','poi.*')
            ->where('poi.tour_id','=',$tour->id)
            ->get();

        // $poi_details = Poi::where('tour_id','=',$tour->id)->get();
        $poi_details = $poi_details->groupBy('poi_id');
    ...
    // public function add_variation_old(Request $request)
    // {
    ...
    // public function edit_variation_old(Request $request)
    // {
    //     $postData = $request->all();
    //     if($postData['variation_name'] == "" ){
    //         return response()->json(['status' => 'failed', 'message' =>'Variation Name cannot be empty.']);
    ...
    // public function delete_variation_old(Request $request)
    // {
    //     $Data = $request->all();
    ...
    public function add_variation(Request $request, Tours $tour)
    {
        ...
        if($request->isMethod('post'))
        {
            ...
            else
            {
                ...
                return back()->with('flash_message_success','Variation created successfully');
            }
    ...
    // public function add_poi_old(Request $request)
    // {
    //     $postData = $request->all();
    //     // echo "<pre>";
    //     // print_r($postData);
    ...
    //         {
    //             if (!empty($postData) && $postData['content_type'] == 'image' && $request->hasFile('image_'.$key)) 
    //             {
    //                 $rules =array();
    //                 $rules['image_'.$key] = 'required|image|mimes:png|max:2048';
    //                 $validator = Validator::make($postData, $rules);
    //                 if ($validator->fails())
    //                 {
    ...
    //                             'image'             =>$image, 
    //                             'poi_id'            =>$postData['poi_unique_id'],
    //                             'lat'               =>$postData['lat'], 
    //                             'long'              =>$postData['long'], 
    //                         );
    //                         $users = Poi::create($insert_data);
    //                     }
    //                 }
    //             }
    //             elseif (!empty($postData) && $postData['content_type'] == 'video' && $request->hasFile('image_'.$key)) 
    //             {
    //                 $rules =array();
    //                 $rules['image_'.$key] = 'required|mimetypes:video/mp4|max:10000';
    //                 $messages = array(
    //                     'image_'.$key.'mimetypes' => 'The Video must be a file of type mp4.'
    //                     );
    //                 $validator = Validator::make($postData, $rules);
    //                 if ($validator->fails())
    //                 {
    ...
    //     else
    //     {
    //         return response()->json(['status' => 'failed', 'message' =>"Please select image"]); 
    //     }
    //     return response()->json(['status' => 'success', 'message'=>'Add Successfully.']);
    // }

    // public function edit_poi_old(Request $request)
    // {
    //     $postData = $request->all();
    ...
    //                         'long'              =>$postData['long'], 
    //                     );
    //                     Poi::where(['id'=>$key,'tour_id'=>$postData['tour_id']])->update($update_data);
    //                 }
    //             }
    //         }
    //         elseif (!empty($postData) && $postData['content_type'] == 'video' && $request->hasFile('image_'.$key)) 
    ...
    //     return response()->json(['status' => 'success', 'message'=>'Update Successfully.']);
    // }
    // public function delete_poi_old(Request $request)
    // {
    //     $Data = $request->all();
    //     $delete_data = array('tour_id' => $Data['tour_id']);
    //     foreach ($Data['id'] as $key => $value) {
    //         $delete_data['id'] = $value;
    //         Poi::where($delete_data)->delete();
    //     }
    //     return response()->json(['success'=>'Delete Successfully.']);
    // }
