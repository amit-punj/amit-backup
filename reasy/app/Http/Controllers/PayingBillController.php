...
class PayingBillController extends Controller {
    ...
    public function payUnitRent($id) {
        ...
        $rentalDate = Carbon::parse($rentDetail->date);
        $rentalDateDayDiff = $rentalDate->diffInDays($today);

        $contract = Booking::find($rentDetail->contract_id);
        $payment_for = 'payUnitRent'; 
        $postData = Input::all();
        $amount = $rentDetail->rent;
        $tenant_id  = Auth::user()->id;
        $user       = Auth::user();
        return view('contracts.check-out',compact('payment_for', 'user', 'amount', 'rentDetail','contract','rentalDateDayDiff'));
    }
    public function payMeterBill($id) {
        $readingDetail = MeterReadings::find($id);

        $today = Helper::Date(Carbon::now()->format('Y/m/d') );
        $rentalDate = Carbon::parse($readingDetail->reading_date);
        $rentalDateDayDiff = $rentalDate->diffInDays($today);

        $contract = Booking::where('status',6)
                                    ->where('start_date', '<', $today)
                                    ->where('end_date', '>=', $today)
                                    ->where('unit_id', $readingDetail->unit_id)
                                    ->first();
        ...
        $tenant_id  = Auth::user()->id;
        $user       = Auth::user();
        return view('contracts.check-out',compact('payment_for', 'user', 'amount', 'readingDetail','rentalDateDayDiff','contract'));
    }
    ...
    public function stripePayUnitRent(Request $request){
        if($request->stripeToken){
            ...
            if($return->status == 'succeeded'){
                ...
                $task = Task::where('rent_id' , $request->get('rent_id'))->first();
    ...
    public function stripePayMeterBill(Request $request){
        if($request->stripeToken){
            ...
            if($return->status == 'succeeded'){
                ...
                $task = Task::where('MeterReading_id' , $request->get('reading_id'))->first();
                $task->status = 'Complete';
                $task->save();
    ...
    public function bankPayRent(Request $request){
        if($request->get('payment_method') == 'bank'){
            ...
            toastr()->success('Unit Rent paid successfully!');
            return redirect('my-wallet');
        }
        ...
        if($request->get('payment_method') == 'paypal'){
            ...
            $transaction = new Transaction();
            $transaction->setAmount($amount) ->setItemList($item_list) ->setDescription('Pay rent');
    ...
    public function paypalUnitrentReturn(Request $request){
        ...
        if ($result->getState() == 'approved') { 
            ...
            $task = Task::where('rent_id' , $billinfo['rent_id'])->first();
            $task->status = 'Complete';
            $task->save();
