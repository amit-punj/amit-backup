...
@include('layouts.banner', ['banner_text' => 'Property Details'])
            ...
            <div class="col-sm-6">
                ...
                <section class="slider">
                    <div id="slider" class="flexslider">
                        <ul class="slides">
                            <li>
                                <a class="fancybox" href="{{ url($url) }}"  data-fancybox-group="gallery"><img src="{{ url($url) }}" alt="" /></a>
                            </li>
                            @if(count($images) > 0)
                                @foreach($images as $key=>$image)
                                    @if($image != "")
                                    <li>
                                        <a class="fancybox" href="{{ url('/images/property_images/540X380/'.$image) }}"  data-fancybox-group="gallery"><img src="{{ url('/images/property_images/540X380/'.$image) }}" alt="" /></a>
                                    </li>
                                    @endif
                                @endforeach
                            @endif
                ...
                <div class="property_info">
                    ...
                    <div class="property_info_body">
                        ...
                        <div class="extra_info">
                            <div class="rent_info unit_size"><span>Type :</span>{{ $data->u_type." (".$data->unit_category.")" }}</div>
                            <div class="rent_info unit_size"><span></span></div>
                            <div class="rent_info unit_size"><span>Size({{$data->area_in}}) :</span>{{ $data->area }}</div>
                            <div class="rent_info"><span>Cost Provision :</span> {{App\Helpers\Helper::CURRENCYSYMBAL.$data->cost_provision}}</div>
                            <div class="rent_info"><span>Rent :</span> {{App\Helpers\Helper::CURRENCYSYMBAL.$data->rent}}</div>
                            <div class="rent_info"><span>Deposit :</span> {{App\Helpers\Helper::CURRENCYSYMBAL.$data->deposit}}</div>
                        </div>
                        ...
                        <div class="extra_info section">
                            <?php 
                            $amenities_all = [];
                            $explode = explode(',', $data->amenities); ?>  
                            @if(count($amenities) > 0)
                                @foreach($amenities as $key => $amenity )
                                    @if(in_array($amenity->id, $explode) )
                                        <?php  $amenities_all[] =  $amenity->amenities_name; ?>
                                    @endif 
                                @endforeach
                                @if(count($amenities_all) > 0)
                                <div class="amenities_title">Amenities/Facilities</div>
                                <div class="amenities_info">
                                    {{ implode(', ', $amenities_all) }}
                                </div>
                                @endif
                            @endif
                        </div>
                ...
                @if((Auth::guest() || Auth::user()->user_role == 1) && $data->booking_status == 0)
                    <div class="property-action"> 
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <button data-toggle="modal" data-target="#send-message">Ask Question</button>
        ...
        <div class="row">
            <div class="col-sm-12">
                <ul class="nav nav-tabs">
                    ...
                    <li><a data-toggle="tab" href="#menu_charges">Charges</a></li>
                    @if (!Auth::guest())
            ...
            <div class="col-sm-12">
                <div class="tab-content">
                    ...
                    <div id="menu_charges" class="tab-pane fade">
                        <div class="row">
                            <div class="col-sm-6">
                                @if(count($meters) > 0)
                                    @foreach($meters as $key => $value)
                                        @if($value->meter_type == 'electric_meter')
                                            <div class="unit"><span>Electricity Meter : </span>{{App\Helpers\Helper::CURRENCYSYMBAL}} {{ $value->unit_price }} per Unit</div>
                                        @endif
                                        @if($value->meter_type == 'water_meter')
                                            <div class="unit"><span>Water Meter : </span>{{App\Helpers\Helper::CURRENCYSYMBAL}} {{ $value->unit_price }} per Unit</div>
                                        @endif
                                        @if($value->meter_type == 'gas_meter')
                                            <div class="unit"><span>Gas Meter : </span>{{App\Helpers\Helper::CURRENCYSYMBAL}} {{ $value->unit_price }} per Unit</div>
                                        @endif
                                    @endforeach
                                @endif
                            </div>
                                ...
                                @foreach($contracts as $key => $contract)
                                    <div class="col-sm-4">
                                        <div class="unit-body">
                                            ...
                                            <div class="unit"><span>Contract With(Tenent) : </span> <a href="{{url('tenant-details/'.$contract->tenant_id)}}"></a>{{ucfirst($contract->user->name)}} </div>
                                            <div class="unit"><span>Contract Type : </span> {{ ucfirst($contract->contract_type)}} </div>
                                            <div class="unit"><span>Starting Date : </span>  {!! \Helper::Date($contract->start_date); !!}</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post" action="{{ url('/send-message/'.$data->id) }}" id="send_message_form">
                    ...
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h3 class="modal-title">Send Messages</h3>
                    </div>
                    <div class="modal-body">
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            @if (Auth::guest())  
                                ...
                                <div class="col-md-3">
                                    <span id="m_send_otp" class="">Send Verification Code</span>
                                </div>
                                <div class="col-md-2">
                        ...
                        <div class="form-group row">
                            <label for="enter_otp" class="col-md-3 col-form-label text-md-right">{{ __('Enter Verification Code') }}</label>
                            <div class="col-md-4">
                                <input id="m_enter_otp" type="text" class="form-control"  name="enter_otp" value="" placeholder="Enter Verification Code">
                            </div>
                            <div class="col-md-3">
                        ...
                        @else
                            <input id="m_enter_otp" type="hidden" class="form-control"  name="enter_otp" value="1" placeholder="Enter Verification Code">
                        @endif
    ...
    <style type="text/css">
        ...
        .flexslider {margin: 0 }
        .rent_info.unit_size {text-transform: capitalize; }
        .content {width: 100%; word-break: break-all; }
        .flex-next{position: absolute; top: 50%; right: 0; z-index: 999; }
        .flex-prev {position: absolute; top: 50%; left: 0; z-index: 999; }
