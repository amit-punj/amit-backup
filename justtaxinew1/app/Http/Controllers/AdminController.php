...
class AdminController extends Controller
{
    ...
    {
        try {
            $str = file_get_contents('http://www.visionvivante.com');
    ...
    public function privacy(){
      ...
      public function TermsANDConditions(){
        return view('admin.pages.TermsANDConditions')
            ->with('title',"Terms and Conditions")
            ->with('page', "TermsANDConditions");
    }
 ...
 /**
   ...
   /* public function assignProviderForm(){
        
        $service_type = ServiceType::get()->pluck('name')->toArray();
        print_r($service_type); die('qwerty');
        return view('admin.dispatch.add', compact('service_type'));
    }
*/

    public function update_schedule_ride()
    {
        $postData = input::all();

        \DB::enableQueryLog();
        $UserRequest = DB::table('user_requests')->where('status','SCHEDULED')
                        ->where('schedule_at','<=',\Carbon\Carbon::now()->addMinutes(10))
                        ->get();

        $hour =  \Carbon\Carbon::now()->subHour();
        $futurehours = \Carbon\Carbon::now()->addMinutes(10);
        ...
        if(!empty($UserRequest)){
            foreach($UserRequest as $ride){
                $ActiveProviders = ProviderService::AvailableServiceProvider($ride->service_type_id)
                        ->get()
                        ->pluck('provider_id');
 ...
 // print_r($query);
                ...
                DB::table('user_requests')
                        ->where('id',$ride->id)
                        ->update(['status' => 'STARTED', 'assigned_at' =>Carbon::now() , 'schedule_at' => null, 'current_provider_id' => $Providers[0]->id ]);
