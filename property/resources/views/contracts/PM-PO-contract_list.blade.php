...
<div class="container">
                                    ...
                                    <tbody >
                                        @foreach($contracts as $key1 => $contract)
                                            <tr> 
                                                ...
                                                <td >
                                                    @php 
                                                        $diff = strtotime($contract->end_date) - strtotime(date('Y/m/d'));
                                                        // 1 day = 24 hours 
                                                        // 24 * 60 * 60 = 86400 seconds 
                                                        $days = (round($diff / 86400))
                                                    @endphp
                                                    @if( $days < 30 )
                                                        <span style="color: red;">{!! \Helper::Date($contract->end_date); !!}</span>
                                                    @elseif( $days < 60 )
                                                        <span style="color: orange;">{!! \Helper::Date($contract->end_date); !!}</span>
                                                    @else
                                                        <span>{!! \Helper::Date($contract->end_date); !!}</span>
                                                    @endif
                                                </td>
                                                ...
                                                ?>
                                                <?php $saleId = (isset($contract->transactions->saleId))? $contract->transactions->saleId : '' ; ?>
                                                @if( ($contract->status == '0' && $contract->payment_status == 'paid' ) || ($contract->status == '0' && $contract->payment_status == 'hold' ))
                                                    ...
                                                    <button data-id="{{$contract->id}}" data-amount="{{$contract->total_amount}}" data-saleId="{{$saleId}}" data-method="{{$contract->payment_method}}" data-status="4" class="booking_action btn btn-danger">Reject</button><br>
                                                    <a href="javascript::void();" data-id="{{$contract->id}}" data-method = {{$payment_method}} data-amount="{{$contract->total_amount}}" data-account="{{$account_number}}" data-router="{{$routing_number}}" data-aba="{{$ada_number}}" data-bank="{{$bank_name}}" data-unit="{{$unit_name}}" data-email="{{$paypal_email}}" data-tenant="{{$contract->user['name']}}" class="payment bank_details">Refund details!</a>
    ...
    <div class="modal fade" id="paypal_refund" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
            <form action="{{ url('paypal-refund-payment') }}" method="POST"  id="add_money_form">
                    @csrf
                    <input type="hidden" name="t_amount" id="t_amount" value="">
                    <input type="hidden" name="id" id="id" value="">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h3 class="modal-title">Add Refund Amount</h3>
                    </div>
                      <div class="modal-body">
                        <div class="row make_payment">
                            <label for="email_token" class="col-md-1 col-form-label text-md-right"></label>
                            <div class="col-md-8">
                                <p id="less_than_amount"></p>
                            </div>
                            <div class="col-md-1">
                            </div>
                        </div> 
                        <div class="row make_payment" style="margin-bottom: 20px;">
                            <label for="email_token" class="col-md-2 col-form-label text-md-right">{{ __('Amount') }}</label>
                            <div class="col-md-8">
                                <input id="amount" placeholder="Amount" type="number" class="form-control"  name="amount" value="">
                            </div>
                            <div class="col-md-1">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-success" id="">Proceed to Refund</button>
                        </div>
                      </div>
                </form>
            </div>
...
<script>
    ...
    jQuery('.booking_action').click(function(){
        ...
        var saleId  = jQuery(this).data('saleId');
        ...
        if(status == '3'){
            ...
            if (result) {
                if(method == 'paypal')
                {
                    $('#paypal_refund').modal('show');
                    $("#paypal_refund #t_amount").val( amount );
                    $("#paypal_refund #id").val( id );
                    $("#paypal_refund #less_than_amount").text("Please refund amount less than: "+amount);
                    // var abc = "{{url('paypal-refund-payment')}}";
                    // window.location.href = abc+"/"+id+"?amount="+selection;
                    return false;
                }
            }
        }
