...
class adminController extends Controller
{
	...
	public function login(Request $request)
	{
		if($request->isMethod('post')){
			...
			if ($validator->fails()){
				...
				}elseif(Auth::attempt(['email' => $data['email'], 'password' => $data['password'],'role' => '2'])){
					return redirect('dashboard');
				}else{
	...
	public function dashboard()
	{
		...
		$total_agents 		= count(User::where('role','=','3')->get());
		$total_customers 	= count(User::where('role','=','4')->get());
		$total_requirements = count(Requirement::get());
		$total_properties	= count(property_list::get());
		return view('admin/dashboard', compact('total_users','total_agents','total_customers','total_requirements','total_properties','sidebar'));
	}
	public function createuser(Request $request)
	{
		...
		if($request->isMethod('post'))
		{
           ...
           $user->save();
           return back()->with('flash_message_success','user Successfully added');
        }
		return view('admin/user/addusers',compact('sidebar'));
	}
    ...
    public function editAdminprofile(Request $request)
    {
                    ...
                    else
                    {
                        ...
                        if($request->file !="")
                        {
                            ...
                            if($request->hasFile('file'))
                            {
                                ...
                                $target_file = $files->getClientOriginalName();
                            }
                        }
                     ...
                     {
                        User::where(['id'=>$getData->id])->update(['profile_pic' => $target_file]);
                     }
    	...
    	return view('admin.setting.edit-profile')->with(compact('AdminDetails','sidebar'));
    }
    ...
    public function generalSetting(Request $request)
    {
        ...
        if($request->isMethod('post') && isset($_POST['siteSave']))
        {
            ...
            if($getData)
            {
                ...
                if ($request->hasFile('site_video')) 
                {
                    $rules['site_video'] = 'required|mimetypes:video/mp4|max:10000';
                }
                else
                {
                    $site_video = $getData->site_video;
                }
                $validator = Validator::make($postData, $rules);
                ...
                else
                {
                    ...
                    if ($request->hasFile('site_video')) 
                    {
                        File::delete('video/' . $getData->site_video);
                        $image = $request->file('site_video');
                        $path = public_path(). '/video/';
                        if (!file_exists($path)) 
                        {
                            File::makeDirectory($path, $mode = 0777, true, true);
                        }   
                        $site_video = time() . '.' . $image->getClientOriginalExtension();
                        $image->move($path, $site_video);
                    }
        ...
        if($getData)
        {
            return view('admin.setting.general')->with(compact('getData','countries','sidebar'));
        }
        else
        {
            return view('admin.setting.general')->with(compact('countries','sidebar'));
        }      
    }
